...
#define CLICKS_PER_SECOND 100  // Increased for finer granularity in time slices
...
static uint32_t time_slice = 10;  // Define time slice in terms of clock ticks
...
static void clock_interrupt(int i, int code)
{
	...
	// Decrease the remaining time slice for the current process
	time_slice--;

	// Check if the time slice has expired
	if (time_slice == 0) {
		time_slice = 10;  // Reset the time slice for the next process
		process_preempt();  // Perform a context switch
	}

	// Increment seconds and reset clicks after a full second
